...
public:
    ...
    int subcount (vector<int>& nums, int idx, int g1, int g2, vector<vector<vector<int>>> &dp) {
        ...
        int take1 = 0;
        if(g1 != 0) take1 += subcount(nums,idx+1,gcd(g1,nums[idx]),g2,dp);
        else take1 += subcount(nums,idx+1,nums[idx],g2,dp);
        int take2 = 0;
        if(g2 != 0) take2 += subcount(nums,idx+1,g1,gcd(g2,nums[idx]),dp);
        else take2 += subcount(nums,idx+1,g1,nums[idx],dp);
